...
class PantryItem {
  ...
  // Get all pantry items for a user with filters
  ...
  static async getExpiringSoon(userId, days = 7) {
    const result = await db.query(
      "SELECT * FROM pantry_items WHERE user_id = $1 AND expiry_date IS NOT NULL AND expiry_date <= CURRENT_DATE + INTERVAL '1 day' * $2 AND expiry_date > CURRENT_DATE ORDER BY expiry_date ASC",
      [userId, days],
    );
  ...
  static async getStats(userId) {
    const result = await db.query(
      "SELECT COUNT(*) AS total_items, COUNT(DISTINCT category) AS total_categories, COUNT(*) FILTER (WHERE is_running_low = true) AS running_low_count, COUNT(*) FILTER (WHERE expiry_date IS NOT NULL AND expiry_date <= CURRENT_DATE + INTERVAL '1 day' * 7 AND expiry_date > CURRENT_DATE) AS expiring_soon_count FROM pantry_items WHERE user_id = $1",
      [userId],
    );
